...
import OTMonthlyPrompt from '../components/OTMonthlyPrompt';
...
export default function DashboardLayout() {
  ...
  const typeToPath: Record<string, string> = {
    ...
    leave_announced: '/leave-calendar',
  ...
  useEffect(() => {
    ...
    if (user?.role) {
      ...
      const handleProfileUpdate = () => { fetchProfilePhoto(); checkProfileCompletion(); };
  ...
  const fetchProfilePhoto = async () => {
    try {
      ...
      if (res.data.profilePhoto) {
        setProfilePhoto(`${API_BASE}${res.data.profilePhoto.url}`);
      }
  ...
  const handleNotificationClick = (notif: Notification) => {
    ...
    switch (notif.type) {
      ...
      case 'leave_announced': navigate('/leave-calendar'); break;
          ...
          <div className="flex items-center gap-4">
            ...
            >
              ...
              {showProfileMenu && (
                ...
                <div className={`w-64 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl shadow-xl border`}>
