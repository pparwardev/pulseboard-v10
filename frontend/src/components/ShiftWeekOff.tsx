...
export default function ShiftWeekOff({ initialShiftStart, initialShiftEnd, initialWeekOff, theme = 'light', onSave }: Props) {
  ...
  const toggleDay = (d: number) => setWeekOff(prev => {
    if (prev.includes(d)) return prev.filter(x => x !== d);
    if (prev.length >= 2) { toast.error('Only 2 week offs can be selected'); return prev; }
    return [...prev, d];
  });

  const calcShiftEnd = (start: string) => {
    if (!start) return '';
  ...
  const handleShiftStartChange = (val: string) => {
    setShiftStart(val);
    setShiftEnd(calcShiftEnd(val));
  };

  const save = async () => {
    if (weekOff.length !== 2) { toast.error('Please select exactly 2 week off days'); return; }
    if (!shiftStart) { toast.error('Please set shift start time'); return; }
    setSaving(true);
    try {
      await api.put('/api/user-preferences/shift-weekoff', {
        shift_start: shiftStart, shift_end: calcShiftEnd(shiftStart), week_off: weekOff.join(','),
      });
      setEditing(false);
    ...
    return (
      <div className="flex items-center gap-3 bg-white rounded-xl px-4 py-2.5 shadow-lg border border-gray-200">
        <div className="flex items-center gap-1.5">
          ...
          <input type="time" value={shiftStart} onChange={e => handleShiftStartChange(e.target.value)}
          ...
          <input type="time" value={shiftEnd} disabled
            className="text-xs border border-gray-300 rounded-lg px-2 py-1 w-24 bg-gray-200 text-gray-600 cursor-not-allowed" />
        </div>
