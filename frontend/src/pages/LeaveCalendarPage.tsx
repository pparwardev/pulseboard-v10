...
export default function LeaveCalendarPage() {
  ...
  useEffect(() => { loadLeaves(); loadTeamLeaves(); }, [currentMonth, currentYear]);
    ...
    <div className="p-6 max-w-[1600px] mx-auto">
              ...
              return (
                <div key={l.id} className={`rounded-lg border-l-4 border-${c}-500 p-3 bg-gray-50`}>
                  ...
                  <div className="flex gap-2 mt-2 items-center">
                    {l.is_announced ? (
                      <span className="text-[10px] text-gray-400">🔒 Announced</span>
                    ) : (
                      <>
      ...
      <div className="w-1/4 flex flex-col min-w-0 bg-white rounded-xl shadow-sm border overflow-hidden">
        <div className="px-4 py-3 border-b bg-gradient-to-r from-purple-600 to-blue-600">
          <h2 className="text-sm font-bold text-white">👥 Team Leaves — {MONTHS[currentMonth]}</h2>
        </div>
        {(() => {
          const monthTeamLeaves = teamLeaves.filter(l => {
            const start = l.leave_date;
            const end = l.end_date || l.leave_date;
            return start <= monthEnd && end >= monthStart;
          });
          if (monthTeamLeaves.length === 0) return (
            <div className="flex-1 flex flex-col items-center justify-center text-gray-400 p-6" style={{ minHeight: '200px' }}>
              <div className="text-4xl mb-2 opacity-40">📋</div>
              <p className="text-xs font-medium text-gray-500">No team leaves in {MONTHS[currentMonth]}</p>
            </div>
          );
          return (
            <div className="overflow-y-auto p-3 space-y-2" style={{ maxHeight: 'calc(100vh - 220px)' }}>
              <p className="text-xs text-gray-500 font-medium px-1">{monthTeamLeaves.length} leave{monthTeamLeaves.length > 1 ? 's' : ''} this month</p>
              {monthTeamLeaves.map((tl: any) => (
                <div key={tl.id} className={`flex items-center gap-3 rounded-lg px-3 py-3 border hover:shadow-sm transition-shadow ${expandedDate && tl.leave_date <= expandedDate && (tl.end_date || tl.leave_date) >= expandedDate ? 'bg-purple-50 border-purple-200' : 'bg-gray-50'}`}>
                  {tl.profile_picture ? (
                    <img src={`${tl.profile_picture}`} alt={tl.user_name}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-gray-400 mt-0.5">
                      {new Date(tl.leave_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })}
                      {tl.end_date && tl.end_date !== tl.leave_date && (
                        <span> → {new Date(tl.end_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })}</span>
                      )}
                    </p>
                  </div>
                </div>
