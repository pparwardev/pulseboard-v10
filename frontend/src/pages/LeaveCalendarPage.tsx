  ...
  return (
    <div className="p-6 max-w-[1600px] mx-auto">
      ...
      <div className="w-1/4 flex flex-col min-w-0">
        <h2 className="text-lg font-bold text-gray-900 mb-3">My Leaves</h2>
        <div className="flex-1 overflow-y-auto pr-1 space-y-3">
          {filteredLeaves.length === 0 ? (
            ...
            filteredLeaves.map(l => {
              ...
              return (
                <div key={l.id} className={`bg-white rounded-xl shadow-sm border-l-4 border-${c}-500 p-4`}>
                  <div className="flex justify-between items-start mb-2">
                    ...
                    {l.is_announced && <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full">✅ Announced</span>}
                  </div>
                  <div className="text-lg font-bold text-gray-900">
                    ...
                    {l.end_date && l.end_date !== l.leave_date && (
                      <span className="text-sm font-normal text-gray-500"> → {new Date(l.end_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })}</span>
                    )}
                  </div>
                  {l.reason && <p className="text-sm text-gray-600 mt-1">{l.reason}</p>}
                  <div className="flex gap-2 mt-3 items-center">
                    {l.is_announced ? (
                      ...
                      <>
                        <button onClick={() => handleAnnounce(l.id)} className="flex-1 px-3 py-1.5 bg-green-600 text-white rounded-lg text-sm hover:bg-green-700 font-medium">
                          📢 Announce
                        </button>
                        <button onClick={() => handleEdit(l)} className="px-3 py-1.5 bg-blue-100 text-blue-700 rounded-lg text-sm hover:bg-blue-200">✏️</button>
                        <button onClick={() => handleDelete(l.id)} className="px-3 py-1.5 bg-red-100 text-red-700 rounded-lg text-sm hover:bg-red-200">🗑️</button>
                      </>
                    )}
                  </div>
                </div>
      ...
      <div className="w-1/2 flex flex-col min-w-0 overflow-y-auto">
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow-sm border p-5 mb-6">
          ...
          <div className="flex gap-2">
            ...
            {editingId && <button onClick={resetForm} className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300">Cancel Edit</button>}
          </div>
      ...
      {/* RIGHT PANEL - Expanded Date (25%) */}
      <div className="w-1/4 flex flex-col min-w-0">
        {expandedDate ? (() => {
          const dayNum = parseInt(expandedDate.split('-')[2]);
          const teamOnLeave = getTeamLeavesForDay(dayNum);
          if (teamOnLeave.length === 0) return <div className="bg-white rounded-xl shadow-sm border p-8 text-center text-gray-400">No details</div>;
          return (
            <div className="bg-white rounded-xl shadow-sm border flex flex-col h-full">
              <div className="flex items-center justify-between px-4 py-3 border-b bg-gradient-to-r from-purple-600 to-blue-600 rounded-t-xl">
                <h3 className="font-bold text-white text-sm">
                  📋 {new Date(expandedDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })}
                </h3>
                <button onClick={() => setExpandedDate(null)} className="text-white/70 hover:text-white text-lg">✕</button>
              </div>
              <div className="flex-1 overflow-y-auto p-3 space-y-2">
                <p className="text-xs text-gray-500 font-medium px-1 mb-1">{teamOnLeave.length} team member{teamOnLeave.length > 1 ? 's' : ''} on leave</p>
                {teamOnLeave.map((tl: any) => (
                  <div key={tl.id} className="flex items-center gap-3 bg-white rounded-lg px-3 py-3 border hover:shadow-sm transition-shadow">
                    {tl.profile_picture ? (
                      <img src={`http://localhost:8001${tl.profile_picture}`} alt={tl.user_name}
          ...
          <div className="bg-gradient-to-b from-purple-50 to-white rounded-xl shadow-sm border h-full flex flex-col items-center justify-center text-gray-400 p-6">
            <div className="text-5xl mb-3 opacity-40">📋</div>
            <p className="text-sm font-medium text-gray-500">Team Leave Details</p>
            <p className="text-xs text-gray-400 mt-1">Click a date with avatars to expand</p>
          </div>
        )}
      </div>{/* end right */}
