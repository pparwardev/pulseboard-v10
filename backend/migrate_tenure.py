...
def parse_date(s):
    """Parse 'September 4, 2017' -> date"""
    m = re.match(r"(\w+)\s+(\d+),\s+(\d{4})", s.strip())
    if not m:
        return None
    month = MONTH_MAP.get(m.group(1).lower())
    return datetime(int(m.group(3)), month, int(m.group(2))).date() if month else None
    ...
    for block in blocks:
        ...
        for i, line in enumerate(lines):
            ...
            if low == "login" and i + 1 < len(lines):
                ...
                data["latest_hire_date"] = lines[i + 1]
...
def main():
    ...
    for rec in records:
        ...
        if not user:
            print(f"  SKIP: login '{rec['login']}' not found in DB")
            continue

        if rec.get("employee_id"):
            user.employee_id = rec["employee_id"]
        if rec.get("latest_hire_date"):
            user.latest_hire_date = parse_date(rec["latest_hire_date"])
        ...
        updated += 1
        print(f"  OK: {rec['login']} -> emp_id={rec.get('employee_id')}, hire={rec.get('latest_hire_date')}, tenure={rec.get('total_tenure')}")
