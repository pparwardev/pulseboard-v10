...
def get_published_metric_data(
    ...
    for r in records:
        ...
        if not member:
            continue
        photo = photo_url(user_map.get(member.user_id))
        members.append({
...
def get_my_performance(
    ...
    return {
        ...
        "photo_url": photo_url(db.query(User).filter(User.id == member.user_id).first()) if member.user_id else None, "overall_score": overall,
