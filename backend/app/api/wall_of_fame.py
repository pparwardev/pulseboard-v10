...
def _build_post_response(post, db, current_user_id):
    ...
    comments = [{"id": c.id, "user_name": c.user.name, "user_photo": c.user.profile_picture, "content": c.content, "created_at": c.created_at.isoformat()}
    ...
    if post.recipient_ids:
        ...
        recipient_names = [{"id": u.id, "name": u.name, "photo": u.profile_picture} for u in users]
    return {
        "id": post.id, "user_id": post.user_id, "user_name": post.user.name, "user_photo": post.user.profile_picture,
...
def search_team_members(q: str = "", db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    return [{"id": u.id, "name": u.name, "photo": u.profile_picture, "login": u.login} for u in query.order_by(User.name).limit(20).all()]
