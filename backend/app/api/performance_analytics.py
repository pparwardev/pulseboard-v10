...
def get_simple_view(week: int = None, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if not members:
        ...
        return {
            ...
            "team_name": current_user.team_name, "manager_name": current_user.name, "manager_photo_url": manager.profile_picture if manager else None, "manager_login": current_user.login
        }
    ...
    user_photos = {u[0]: u[1] for u in db.query(User.id, User.profile_picture).filter(User.id.in_([m.user_id for m in members if m.user_id])).all()}
    ...
    for member in members:
        ...
        if mc:
            ...
            result.append({
                ...
                "photo_url": member.photo_url or user_photos.get(member.user_id),
    ...
    return {
        ...
        "team_name": current_user.team_name, "manager_name": manager.name if manager else "", "manager_photo_url": manager.profile_picture if manager else None, "manager_login": manager.login if manager else ""
    }
