...
def announce_leave(leave_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if manager:
        create_notification(db=db, user_id=manager.id, title='Leave Announced', message=msg, notification_type='ot_leave_submitted')
    # Notify all team members
    team_ids = _get_team_ids(db, current_user)
    for tid in team_ids:
        if tid != current_user.id and (not manager or tid != manager.id):
            create_notification(db=db, user_id=tid, title='Team Leave Update', message=msg, notification_type='ot_leave_submitted')
...
def cancel_leave(leave_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    manager = db.query(User).filter(User.team_name == current_user.team_name, User.role == 'manager').first()
    if manager:
        create_notification(db=db, user_id=manager.id, title='Leave Cancelled', message=msg, notification_type='ot_leave_submitted')
    team_ids = _get_team_ids(db, current_user)
    for tid in team_ids:
        if tid != current_user.id and (not manager or tid != manager.id):
            create_notification(db=db, user_id=tid, title='Leave Cancelled', message=msg, notification_type='ot_leave_submitted')
    db.delete(leave)
    db.commit()
