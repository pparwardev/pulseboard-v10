...
def get_live_team_strength(db: Session, team_name: str = None):
    ...
    # Count members on week off today
    on_weekoff = 0
    ...
    present = max(0, total - on_leave - on_weekoff)
    return {"total_members": total, "present": present, "on_leave": on_leave, "on_weekoff": on_weekoff}
...
def get_member_dashboard(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    return {
        "user": {
            ...
            "profile_picture": photo_url(current_user),
